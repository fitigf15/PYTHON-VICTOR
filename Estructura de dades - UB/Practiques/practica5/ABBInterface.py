...
class ABBNode:
	...
	if isinstance(self.val,Stack):
	    self.val.push(value)
	else:
	    self.val=Stack(self.val)
	    self.val.push(value)
	...
	if isinstance(self.val,Stack):
	    val=[]
	    for i in range(self.val.items):
		val.append(str(self.val[i]))
	else:
	    val=self.val
...
class ABB:
	...
	if node is not None:
	    if isinstance(node.val,Stack):
		if node.val.items>1:
		    node.val.pop()
		    self.items-=1
		else:
		    node.val=node.val.pop()
	    if not isinstance(node.val,Stack):
	...
	if self.root is not None:
		...
		if node!=None:
		    if isinstance(node.val,Stack):
			for i in range(node.val.items):
			    yield ABBNode(node.val[i],node.key)
		    else:
			yield node
		    stack.push(node)
		    node=node.left
